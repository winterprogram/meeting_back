...
let adminlogin = (req, res) => {
    ...
    let userLoginFinal = () => {
        return new Promise((resolve, reject) => {
            adminmodel.find({ email: req.body.email }).exec((err, data) => {
                ...
                if (err) {
                    ...
                    passencry.passcheck(req.body.password, data[0].password, ((error, result) => {
                        ...
                        if (error) {
                            ...
                            logger.error('adminmodel doesn\'t have right\'s to access', 'login:userLoginFinal()', 10)
                            let apis = api.apiresponse(true, 'adminmodel doesn\'t have right\'s to access', 503, null)
                            reject(apis)
                        }
                        else if (result == true) {
        ...
        return new Promise((resolve, reject) => {
            jwt.generateToken(userData, ((err, result) => {
                if (err) {
                    ...
                    admintoken.deleteOne({ userid: userData.data[0].userid }).exec((err, result) => {
                        if (err) {
                            logger.error('error while deleting the adminmodel token', 'login:userLoginFinal()', 10)
                            let response = api.apiresponse(true, 'error while deleting the adminmodel token', 504, null)
                            reject(response)
                        } else if (emptyCheck.emptyCheck(result)) {
                            logger.error('error while deleting the adminmodel token', 'login:userLoginFinal()', 10)
                            let response = api.apiresponse(true, 'error while deleting the adminmodel token', 400, null)
                            reject(response)
                        } else {
