...
let usersignupadmin = (req, res) => {
    ...
    let userexist = () => {
        return new Promise((resolve, reject) => {
            adminmodel.find({ email: req.body.email }).exec((err, result) => {
    ...
    let datasave = () => {
        return new Promise((resolve, reject) => {
            adminmodel.find({ mobilenumber: req.body.mobilenumber }).exec((err, result) => {
                if (err) {
                    let response = api.apiresponse(true, 404, 'something went wrong', null)
                    reject(response)
                } else if (emptycheck.emptyCheck(result)) {
...
module.exports = {
    usersignupadmin: usersignupadmin
}